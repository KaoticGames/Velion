...
type AccountSection = 'overview' | 'profile' | 'notifications' | 'billing';

const ACCOUNT_NAV: { id: AccountSection; label: string }[] = [
...
function sectionFromSearch(sp: URLSearchParams): AccountSection {
  if (sp.get('sub') === 'billing') return 'billing';
  const raw = sp.get('section');
  if (raw === 'profile' || raw === 'notifications' || raw === 'billing') return raw;
  if (raw === 'account' || raw === 'overview') return 'overview';
  return 'overview';
}
...
export default function Account() {
  ...
  useEffect(() => {
    if (searchParams.get('sub') !== 'billing') return;
    const q = new URLSearchParams(searchParams);
    q.delete('sub');
    if (!q.get('section')) q.set('section', 'billing');
  ...
  if (loadError && !account) {
    return (
      <div className="page-enter" style={{ color: T.text, fontFamily: "'EB Garamond', serif", padding: '40px 28px', maxWidth: '720px', margin: '0 auto' }}>
        ...
        <button type="button" onClick={() => void reloadAccount()} style={secondaryBtn}>Retry</button>
      </div>
    );
  }
    ...
    <div className="page-enter" style={{
      ...
      <header style={{ marginBottom: '28px' }}>
        ...
        <p style={{ color: T.textMuted, fontSize: '18px', lineHeight: 1.65, maxWidth: '680px', margin: 0 }}>
          Overview (email & password), public profile, notification preferences, and billing.
        </p>
      ...
      {showCheckoutOk && section !== 'billing' && (
      ...
      {showCheckoutCanceled && section !== 'billing' && (
...
function AccountSections(props: {
  ...
  const {
    account, busy, setBusy, navigate, loadAccount, mergeUser, onError, refreshSession,
  } = props;
...
function NotificationsPlaceholder() {
  const rows = [
    ...
    { id: 'billing', label: 'Billing & receipts', desc: 'Receipts, failed payments, and subscription notices.' },
  ] as const;
  ...
  return (
    <section style={sectionBox}>
      ...
      <p style={{ color: T.textMuted, fontSize: '18px', lineHeight: 1.65, marginTop: 0 }}>
        Control email and in-app alerts. <span style={{ color: T.gold }}>Coming soon</span> — delivery and saved preferences are not enabled yet; below is a preview of the categories we plan to support.
      </p>
      <div
        style={{
