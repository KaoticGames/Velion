...
const SectionLabel = ({ children }: { children: ReactNode }) => (
  <div style={{ fontFamily:"'Cinzel',serif", fontSize:'9px', letterSpacing:'0.26em', color:T.textDim, marginBottom:'12px', marginTop:'28px', borderBottom:`1px solid ${T.border}`, paddingBottom:'8px' }}>{children}</div>
);
...
const CopyButton = ({ text }: { text: string }) => {
  ...
  return (
    <button onClick={() => { navigator.clipboard.writeText(text); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
      style={{ fontFamily:"'Cinzel',serif", fontSize:'9px', letterSpacing:'0.14em', background: copied ? T.green+'22' : 'transparent', border:`1px solid ${copied ? T.green : T.border}`, borderRadius:'2px', padding:'4px 10px', cursor:'pointer', color: copied ? T.green : T.textMuted, transition:'all 0.15s', flexShrink:0 }}>
      {copied ? '✓ COPIED' : 'COPY'}
    </button>
  );
};
...
function MembersPanel({ members, campaignId, isDM, currentUserId }: { members: CampaignMember[]; campaignId: string; isDM: boolean; currentUserId: string }) {
  ...
  if (!members.length) return (
    <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:'3px', padding:'20px', textAlign:'center', fontSize:'12px', color:T.textDim, fontFamily:"'Cinzel',serif", letterSpacing:'0.14em' }}>NO PLAYERS YET</div>
  );
        ...
        return (
          <div key={membership.id} style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:'3px', padding:'14px 16px', display:'flex', alignItems:'center', gap:'14px' }}>
            <div style={{ width:'40px', height:'40px', borderRadius:'2px', flexShrink:0, background:T.card, border:`1px solid ${ac}44`, overflow:'hidden', display:'flex', alignItems:'center', justifyContent:'center' }}>
              {character?.portrait_url ? <img src={character.portrait_url} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <span style={{ fontSize:'18px', color:ac, opacity:0.5 }}>⚔</span>}
            </div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'2px' }}>
                <span style={{ fontSize:'14px', fontWeight:'600', color:T.text }}>{character?.name ?? 'Unknown Character'}</span>
                {isMe && <span style={{ fontFamily:"'Cinzel',serif", fontSize:'8px', letterSpacing:'0.18em', color:T.gold, border:`1px solid ${T.goldDim}`, borderRadius:'2px', padding:'1px 5px' }}>YOU</span>}
              </div>
              <div style={{ fontSize:'11px', color:T.textDim }}>{character ? `Level ${character.level} · ${cap(character.chosen_attribute)} · ${user?.email ?? ''}` : user?.email ?? ''}</div>
            </div>
            {character && (
            ...
            {canRemove && (
              <button
                ...
                style={{ fontFamily:"'Cinzel',serif", fontSize:'9px', letterSpacing:'0.12em', background: confirming === membership.character_id ? T.hp : 'transparent', border:`1px solid ${T.hp}`, color: confirming === membership.character_id ? '#080b10' : T.hp, borderRadius:'2px', padding:'4px 10px', cursor:'pointer', flexShrink:0 }}>
...
function ManagementPanel({ campaign, onDelete }: { campaign: any; onDelete: () => void }) {
  ...
  const SaveBtn = ({ disabled, saved, onClick, label }: { disabled: boolean; saved: boolean; onClick: () => void; label: string }) => (
    <button onClick={onClick} disabled={disabled} style={{ fontFamily:"'Cinzel',serif", fontSize:'9px', letterSpacing:'0.14em', background: saved ? T.green : !disabled ? T.gold : T.goldDim, border:`1px solid ${saved ? T.green : T.gold}`, borderRadius:'2px', padding:'6px 16px', cursor: !disabled ? 'pointer' : 'not-allowed', color:'#080b10', fontWeight:'700' }}>
      {saved ? '✓ SAVED' : label}
    </button>
  );

  const inputStyle = { width:'100%', boxSizing:'border-box' as const, background:T.surface, border:`1px solid ${T.border}`, borderRadius:'3px', padding:'10px 14px', color:T.text, fontSize:'13px', lineHeight:'1.6', outline:'none', fontFamily:'inherit' };
  const taStyle    = { ...inputStyle, resize:'vertical' as const };

  return (
    <div>
      <SectionLabel>CAMPAIGN SUMMARY</SectionLabel>
      <p style={{ fontSize:'12px', color:T.textMuted, lineHeight:'1.6', margin:'0 0 8px', fontStyle:'italic' }}>Visible to all members on the campaign page.</p>
      ...
      <p style={{ fontSize:'12px', color:T.textMuted, lineHeight:'1.6', margin:'0 0 8px', fontStyle:'italic' }}>Private — never visible to players.</p>
      ...
      <p style={{ fontSize:'12px', color:T.textMuted, lineHeight:'1.6', margin:'0 0 12px', fontStyle:'italic' }}>Maps, tokens, and reference images. Accessible here and in the VTT.</p>
      <div style={{ marginBottom:'12px', display:'flex', alignItems:'center', gap:'10px' }}>
        <input ref={fileRef} type="file" accept=".png,.jpg,.jpeg,.webp" onChange={handleUpload} style={{ display:'none' }} />
        <button onClick={() => fileRef.current?.click()} disabled={uploading} style={{ fontFamily:"'Cinzel',serif", fontSize:'10px', letterSpacing:'0.14em', background:'transparent', border:`1px solid ${T.gold}`, borderRadius:'3px', padding:'8px 18px', cursor: uploading ? 'not-allowed' : 'pointer', color:T.gold }}>
          {uploading ? '↑ UPLOADING…' : '+ UPLOAD ASSET'}
        </button>
        {uploadError && <span style={{ fontSize:'11px', color:T.hp }}>{uploadError}</span>}
      </div>

      {assets.length === 0
        ? <div style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:'3px', padding:'16px', textAlign:'center', fontSize:'11px', color:T.textDim, fontFamily:"'Cinzel',serif", letterSpacing:'0.14em' }}>NO ASSETS YET</div>
              ...
              return (
                <div key={asset.id} style={{ background:T.surface, border:`1px solid ${T.border}`, borderRadius:'3px', overflow:'hidden', position:'relative' }} onMouseLeave={() => setConfirmDelete(null)}>
                  ...
                  <div style={{ padding:'8px 10px' }}>
                    <div style={{ fontSize:'12px', color:T.text, fontWeight:'600', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', marginBottom:'3px' }}>{asset.name}</div>
                    <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                      <span style={{ fontFamily:"'Cinzel',serif", fontSize:'8px', letterSpacing:'0.14em', color:ac, background:ac+'18', border:`1px solid ${ac}33`, borderRadius:'2px', padding:'1px 6px' }}>{asset.asset_type.toUpperCase()}</span>
                      <span style={{ fontSize:'10px', color:T.textDim }}>{fmtBytes(asset.size_bytes)}</span>
                    </div>
                  </div>
                  <button onClick={() => handleDelete(asset.id)} disabled={deletingId === asset.id} style={{ position:'absolute', top:'6px', right:'6px', fontFamily:"'Cinzel',serif", fontSize:'8px', letterSpacing:'0.1em', background: confirmDelete === asset.id ? T.hp : '#000000aa', border:`1px solid ${confirmDelete === asset.id ? T.hp : T.border}`, borderRadius:'2px', padding:'3px 7px', cursor:'pointer', color: confirmDelete === asset.id ? '#080b10' : T.textMuted }}>
      ...
      <label style={{ fontFamily:"'Cinzel',serif", fontSize:'9px', letterSpacing:'0.2em', color:T.textDim, display:'block', marginBottom:'6px' }}>NAME</label>
      <div style={{ display:'flex', gap:'8px', marginBottom:'20px' }}>
        ...
        <button onClick={saveName} disabled={name.trim() === campaign.name} style={{ fontFamily:"'Cinzel',serif", fontSize:'9px', letterSpacing:'0.14em', background: nameSaved ? T.green : name.trim() !== campaign.name ? T.gold : T.goldDim, border:`1px solid ${nameSaved ? T.green : T.gold}`, borderRadius:'3px', padding:'9px 16px', cursor:'pointer', color:'#080b10', fontWeight:'700' }}>
          {nameSaved ? '✓' : 'SAVE'}
        </button>
      </div>
      <button onClick={() => { if (!confirmDel) { setConfirmDel(true); } else { onDelete(); } }} onBlur={() => setConfirmDel(false)} style={{ fontFamily:"'Cinzel',serif", fontSize:'9px', letterSpacing:'0.14em', background: confirmDel ? T.hp : 'transparent', border:`1px solid ${T.hp}`, borderRadius:'2px', padding:'8px 16px', cursor:'pointer', color: confirmDel ? '#080b10' : T.hp }}>
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'calc(100vh - 52px)', overflow:'hidden' }}>
      ...
      <div style={{ borderBottom:`1px solid ${T.border}`, padding:'20px 32px 0', background:T.surface, flexShrink:0 }}>
        <div style={{ marginBottom:'16px' }}>
          <Link to="/campaigns" style={{ fontFamily:"'Cinzel',serif", fontSize:'10px', letterSpacing:'0.2em', color:T.textDim, textDecoration:'none' }}>← CAMPAIGNS</Link>
        </div>

        <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'16px', gap:'24px' }}>
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'6px' }}>
              <h1 style={{ fontFamily:"'Cinzel',serif", fontSize:'22px', color:tc, margin:'0', letterSpacing:'0.1em', fontWeight:'700' }}>{campaign.name}</h1>
              {isDM && isOwner && <span style={{ fontFamily:"'Cinzel',serif", fontSize:'8px', letterSpacing:'0.18em', color:T.dmGold, background:T.dmGold+'18', border:`1px solid ${T.dmGold}33`, borderRadius:'2px', padding:'2px 6px' }}>DM</span>}
            </div>
            {(campaign as any).summary && <p style={{ fontSize:'13px', color:T.textMuted, lineHeight:'1.6', margin:'0 0 8px', maxWidth:'560px' }}>{(campaign as any).summary}</p>}
            <div style={{ display:'flex', gap:'10px', alignItems:'center', flexWrap:'wrap' }}>
              <span style={{ fontFamily:"'Cinzel',serif", fontSize:'9px', letterSpacing:'0.18em', color:tc, background:tc+'18', border:`1px solid ${tc}44`, borderRadius:'2px', padding:'2px 8px' }}>{campaign.world_tier_baseline.toUpperCase()}</span>
              <span style={{ fontFamily:"'Cinzel',serif", fontSize:'10px', color:T.textDim }}>{campaign.members.length} member{campaign.members.length !== 1 ? 's' : ''}</span>
            </div>
          ...
          {isDM && isOwner && (
            <div style={{ display:'flex', flexDirection:'column', alignItems:'flex-end', gap:'8px', flexShrink:0 }}>
              <button onClick={handleLaunchVTT} disabled={launching} style={{ fontFamily:"'Cinzel',serif", fontSize:'12px', letterSpacing:'0.16em', background: launching ? T.goldDim : T.gold, border:`1px solid ${T.gold}`, borderRadius:'3px', padding:'11px 26px', cursor: launching ? 'not-allowed' : 'pointer', color:'#080b10', fontWeight:'700', display:'flex', alignItems:'center', gap:'8px' }}>
                <span style={{ fontSize:'14px' }}>⚔</span>
                {launching ? 'LAUNCHING…' : 'LAUNCH VTT'}
              </button>
              {launchError && <span style={{ fontSize:'11px', color:T.hp }}>{launchError}</span>}
              {inviteUrl && (
                <div style={{ display:'flex', alignItems:'center', gap:'6px' }}>
                  <code style={{ fontSize:'10px', color:T.rp, background:T.card, border:`1px solid ${T.border}`, borderRadius:'2px', padding:'3px 8px', maxWidth:'200px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{inviteUrl}</code>
                  <CopyButton text={inviteUrl} />
                  <button onClick={handleRefreshInvite} disabled={refreshing} title="Generate a new invite link" style={{ fontFamily:"'Cinzel',serif", fontSize:'9px', background:'transparent', border:`1px solid ${T.border}`, borderRadius:'2px', padding:'4px 7px', cursor:'pointer', color:T.textMuted }}>
                    ↻
                  </button>
                </div>
              )}
